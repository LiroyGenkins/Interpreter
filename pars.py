...
class Node:                 #нетерминал
    def __init__(self, name = '', value = '',height = 0):
        ...
        self.height = height
        self.rpn = []
    def __repr__(self):     #вывод строкового представления
        str = ''
...
class SyntaxCheck:
    ...
    def advance(self):      #продвижение по токенам
            ...
            try:
                ...
                if self.current_tok[1] == 'VAR':
                    try:
                        ...
                        expr.childs.append(linkedlist)
                        self.check_next_t('CLOSE')
                        self.advance()
                        self.to_rpn(linkedlist)
                        expr.rpn = linkedlist.rpn.copy()
                        self.height -= 1
                        return expr
                    except BaseException:
                        self.index = self.ret_index - 1;
    ...
    def assign_expr(self):
        ...
        try:
            ...
            self.height += 1
            self.ret_index = self.index
            if self.current_tok[1] in ('LINKED_LIST'):
                assign_expr.childs.append(self.linkedlist())
            else:
                self.index = self.ret_index - 1
                self.advance()
        ...
        try:
            ...
            for i in range(len(while_expr.rpn)):
                if while_expr.rpn[i] == 'start':
                    while_expr.rpn[i] = (prev,'NUM')
                if while_expr.rpn[i] == 'end':
                    while_expr.rpn[i] = (prev + len(while_expr.rpn),'NUM')
            self.height-=1
            return while_expr
        except BaseException:
            raise BaseException
        ...
        try:
            ...
            for i in range(len(if_expr.rpn)):
                if if_expr.rpn[i] == 'else':
                    if_expr.rpn[i] = (prev + len(if_expr.rpn),'NUM') # подстановка значения для метки перед условием
                if if_expr.rpn[i] == 'end':
                    if_expr.rpn[i] = (prev + len(if_expr.rpn),'NUM') # подстановка значения для метки в конце if
                    end_index = i
            try:
                self.check_next_t(('ELSE'))
                self.advance()
                try:
                    ...
                    for i in range(len(if_expr.rpn)):
                        if i == end_index:
                            if_expr.rpn[i] = (prev + len(if_expr.rpn),'NUM') # если есть else - увеличивает метку end на его длину
                except BaseException:
                    raise BaseException
            except:
                pass
    ...
    def linkedlist(self):
        try:
            llist = Node('linkedlist',height=self.height)
            self.height += 1
            self.check_next_t('LINKED_LIST')
            self.buffer.append(self.current_tok)
            self.advance()
            self.check_next_t('LP')
            self.buffer.append(self.current_tok)
            self.advance()
            llist.childs.append(self.args())
            self.height -= 1
            self.check_next_t('RP')
            self.buffer.append(self.current_tok)
            self.advance()
    ...
    def to_rpn(self, expr):
        parser = SortStation(self.buffer)
        expr.rpn = parser.rpn()
        self.buffer.clear()
...
class SortStation:
    ...
    def rpn(self):
        ...
        while ((len(self.tokens) != 0) | (self.current_tok[1] != None)):
            if (self.current_tok[1] in ('VAR','NUM')):
                if read_vars:
                    var_count += 1
                self.output.append(self.current_tok)
                self.advance()
            elif self.is_linkedlist(self.current_tok):
                read_vars = True
                ...
                self.advance()
                if (len(self.stack) != 0):
                    if self.is_linkedlist(self.stack[-1]):
    ...
    def is_linkedlist(self, t):
        return t[1] == 'LINKED_LIST'